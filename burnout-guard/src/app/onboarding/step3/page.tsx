...
export default function OnboardingStep3Page() {
  ...
  // Redirect to appropriate step if user hasn't completed previous steps
  ...
  const handleSubmit = async (data: any) => {
    ...
    // After completing step 3, redirect to dashboard
    handleComplete();
  };
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-md mx-auto mb-8">
          <div className="flex items-center justify-between">
            {[1, 2, 3].map((step) => (
              <div key={step} className="flex items-center">
                ...
                {step < 3 && (
                  <div className={`w-12 h-1 mx-2 ${step < 3 ? 'bg-gradient-to-r from-purple-600 to-pink-600' : 'bg-gray-200'}`} />
                )}
              </div>
            ))}
          </div>
          <div className="flex justify-between text-xs text-gray-500 mt-2">
            ...
            <span className="font-medium text-purple-600">Goals</span>
          </div>
        </div>
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="max-w-2xl mx-auto mt-8 p-4 bg-gray-100 rounded text-sm">
            <h3 className="font-bold mb-2">Debug Info - Step 3:</h3>
            <p>Current Page: /onboarding/step-3</p>
            <p>User ID: {user?.id}</p>
